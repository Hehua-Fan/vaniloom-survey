import { promises as fs } from 'fs'
import path from 'path'

export interface BetaAccount {
  id: string
  username: string
  password: string
  isAssigned: boolean
  assignedTo?: string
  assignedAt?: Date
}

// 账号状态文件路径
const ACCOUNTS_STATE_FILE = path.join(process.cwd(), 'accounts-state.json')

// 初始账号列表
const initialBetaAccounts: BetaAccount[] = [
  {
    id: '1',
    username: 'Vanitest01',
    password: '162217',
    isAssigned: false,
  },
  {
    id: '2',
    username: 'Vanitest02',
    password: '742743',
    isAssigned: false,
  },
  {
    id: '3',
    username: 'Vanitest03',
    password: '246432',
    isAssigned: false,
  },
  {
    id: '4',
    username: 'Vanitest04',
    password: '657235',
    isAssigned: false,
  },
  {
    id: '5',
    username: 'Vanitest05',
    password: '563300',
    isAssigned: false,
  },
  {
    id: '6',
    username: 'Vanitest06',
    password: '865463',
    isAssigned: false,
  },
  {
    id: '7',
    username: 'Vanitest07',
    password: '043182',
    isAssigned: false,
  },
  {
    id: '8',
    username: 'Vanitest08',
    password: '181813',
    isAssigned: false,
  },
  {
    id: '9',
    username: 'Vanitest09',
    password: '137087',
    isAssigned: false,
  },
  {
    id: '10',
    username: 'Vanitest10',
    password: '336188',
    isAssigned: false,
  },
  {
    id: '11',
    username: 'Vanitest11',
    password: '993211',
    isAssigned: false,
  },
  {
    id: '12',
    username: 'Vanitest12',
    password: '433581',
    isAssigned: false,
  },
  {
    id: '13',
    username: 'Vanitest13',
    password: '009877',
    isAssigned: false,
  },
  {
    id: '14',
    username: 'Vanitest14',
    password: '552119',
    isAssigned: false,
  },
  {
    id: '15',
    username: 'Vanitest15',
    password: '070391',
    isAssigned: false,
  },
  {
    id: '16',
    username: 'Vanitest16',
    password: '731907',
    isAssigned: false,
  },
  {
    id: '17',
    username: 'Vanitest17',
    password: '066934',
    isAssigned: false,
  },
  {
    id: '18',
    username: 'Vanitest18',
    password: '730576',
    isAssigned: false,
  },
  {
    id: '19',
    username: 'Vanitest19',
    password: '754352',
    isAssigned: false,
  },
  {
    id: '20',
    username: 'Vanitest20',
    password: '432230',
    isAssigned: false,
  },
  {
    id: '21',
    username: 'Vanitest21',
    password: '938450',
    isAssigned: false,
  },
  {
    id: '22',
    username: 'Vanitest22',
    password: '509326',
    isAssigned: false,
  },
  {
    id: '23',
    username: 'Vanitest23',
    password: '828102',
    isAssigned: false,
  },
  {
    id: '24',
    username: 'Vanitest24',
    password: '191767',
    isAssigned: false,
  },
  {
    id: '25',
    username: 'Vanitest25',
    password: '444210',
    isAssigned: false,
  },
  {
    id: '26',
    username: 'Vanitest26',
    password: '088583',
    isAssigned: false,
  },
  {
    id: '27',
    username: 'Vanitest27',
    password: '705592',
    isAssigned: false,
  },
  {
    id: '28',
    username: 'Vanitest28',
    password: '398708',
    isAssigned: false,
  },
  {
    id: '29',
    username: 'Vanitest29',
    password: '255620',
    isAssigned: false,
  },
  {
    id: '30',
    username: 'Vanitest30',
    password: '734753',
    isAssigned: false,
  },
  {
    id: '31',
    username: 'Vanitest31',
    password: '548047',
    isAssigned: false,
  },
  {
    id: '32',
    username: 'Vanitest32',
    password: '006300',
    isAssigned: false,
  },
  {
    id: '33',
    username: 'Vanitest33',
    password: '206087',
    isAssigned: false,
  },
  {
    id: '34',
    username: 'Vanitest34',
    password: '427979',
    isAssigned: false,
  },
  {
    id: '35',
    username: 'Vanitest35',
    password: '245196',
    isAssigned: false,
  },
  {
    id: '36',
    username: 'Vanitest36',
    password: '260340',
    isAssigned: false,
  },
  {
    id: '37',
    username: 'Vanitest37',
    password: '970267',
    isAssigned: false,
  },
  {
    id: '38',
    username: 'Vanitest38',
    password: '577372',
    isAssigned: false,
  },
  {
    id: '39',
    username: 'Vanitest39',
    password: '205939',
    isAssigned: false,
  },
  {
    id: '40',
    username: 'Vanitest40',
    password: '656047',
    isAssigned: false,
  },
  {
    id: '41',
    username: 'Vanitest41',
    password: '550110',
    isAssigned: false,
  },
  {
    id: '42',
    username: 'Vanitest42',
    password: '895159',
    isAssigned: false,
  },
  {
    id: '43',
    username: 'Vanitest43',
    password: '554091',
    isAssigned: false,
  },
  {
    id: '44',
    username: 'Vanitest44',
    password: '225031',
    isAssigned: false,
  },
  {
    id: '45',
    username: 'Vanitest45',
    password: '564324',
    isAssigned: false,
  },
  {
    id: '46',
    username: 'Vanitest46',
    password: '959122',
    isAssigned: false,
  },
  {
    id: '47',
    username: 'Vanitest47',
    password: '029167',
    isAssigned: false,
  },
  {
    id: '48',
    username: 'Vanitest48',
    password: '615084',
    isAssigned: false,
  },
  {
    id: '49',
    username: 'Vanitest49',
    password: '244539',
    isAssigned: false,
  },
  {
    id: '50',
    username: 'Vanitest50',
    password: '588234',
    isAssigned: false,
  },
  {
    id: '51',
    username: 'Vanitest51',
    password: '193189',
    isAssigned: false,
  },
  {
    id: '52',
    username: 'Vanitest52',
    password: '804103',
    isAssigned: false,
  },
  {
    id: '53',
    username: 'Vanitest53',
    password: '521123',
    isAssigned: false,
  },
  {
    id: '54',
    username: 'Vanitest54',
    password: '023350',
    isAssigned: false,
  },
  {
    id: '55',
    username: 'Vanitest55',
    password: '691533',
    isAssigned: false,
  },
  {
    id: '56',
    username: 'Vanitest56',
    password: '895377',
    isAssigned: false,
  },
  {
    id: '57',
    username: 'Vanitest57',
    password: '245127',
    isAssigned: false,
  },
  {
    id: '58',
    username: 'Vanitest58',
    password: '888333',
    isAssigned: false,
  },
  {
    id: '59',
    username: 'Vanitest59',
    password: '319934',
    isAssigned: false,
  },
  {
    id: '60',
    username: 'Vanitest60',
    password: '040193',
    isAssigned: false,
  },
  {
    id: '61',
    username: 'Vanitest61',
    password: '383809',
    isAssigned: false,
  },
  {
    id: '62',
    username: 'Vanitest62',
    password: '213511',
    isAssigned: false,
  },
  {
    id: '63',
    username: 'Vanitest63',
    password: '028343',
    isAssigned: false,
  },
  {
    id: '64',
    username: 'Vanitest64',
    password: '686075',
    isAssigned: false,
  },
  {
    id: '65',
    username: 'Vanitest65',
    password: '081000',
    isAssigned: false,
  },
  {
    id: '66',
    username: 'Vanitest66',
    password: '860318',
    isAssigned: false,
  },
  {
    id: '67',
    username: 'Vanitest67',
    password: '383992',
    isAssigned: false,
  },
  {
    id: '68',
    username: 'Vanitest68',
    password: '560040',
    isAssigned: false,
  },
  {
    id: '69',
    username: 'Vanitest69',
    password: '420477',
    isAssigned: false,
  },
  {
    id: '70',
    username: 'Vanitest70',
    password: '308727',
    isAssigned: false,
  },
  {
    id: '71',
    username: 'Vanitest71',
    password: '784907',
    isAssigned: false,
  },
  {
    id: '72',
    username: 'Vanitest72',
    password: '993173',
    isAssigned: false,
  },
  {
    id: '73',
    username: 'Vanitest73',
    password: '065517',
    isAssigned: false,
  },
  {
    id: '74',
    username: 'Vanitest74',
    password: '142126',
    isAssigned: false,
  },
  {
    id: '75',
    username: 'Vanitest75',
    password: '541122',
    isAssigned: false,
  },
  {
    id: '76',
    username: 'Vanitest76',
    password: '469209',
    isAssigned: false,
  },
  {
    id: '77',
    username: 'Vanitest77',
    password: '198943',
    isAssigned: false,
  },
  {
    id: '78',
    username: 'Vanitest78',
    password: '907107',
    isAssigned: false,
  },
  {
    id: '79',
    username: 'Vanitest79',
    password: '541212',
    isAssigned: false,
  },
  {
    id: '80',
    username: 'Vanitest80',
    password: '456424',
    isAssigned: false,
  },
  {
    id: '81',
    username: 'Vanitest81',
    password: '201593',
    isAssigned: false,
  },
  {
    id: '82',
    username: 'Vanitest82',
    password: '539610',
    isAssigned: false,
  },
  {
    id: '83',
    username: 'Vanitest83',
    password: '714838',
    isAssigned: false,
  },
  {
    id: '84',
    username: 'Vanitest84',
    password: '340042',
    isAssigned: false,
  },
  {
    id: '85',
    username: 'Vanitest85',
    password: '963690',
    isAssigned: false,
  },
  {
    id: '86',
    username: 'Vanitest86',
    password: '339689',
    isAssigned: false,
  },
  {
    id: '87',
    username: 'Vanitest87',
    password: '093984',
    isAssigned: false,
  },
  {
    id: '88',
    username: 'Vanitest88',
    password: '024154',
    isAssigned: false,
  },
  {
    id: '89',
    username: 'Vanitest89',
    password: '109667',
    isAssigned: false,
  },
  {
    id: '90',
    username: 'Vanitest90',
    password: '442031',
    isAssigned: false,
  },
  {
    id: '91',
    username: 'Vanitest91',
    password: '556539',
    isAssigned: false,
  },
  {
    id: '92',
    username: 'Vanitest92',
    password: '754828',
    isAssigned: false,
  },
  {
    id: '93',
    username: 'Vanitest93',
    password: '301632',
    isAssigned: false,
  },
  {
    id: '94',
    username: 'Vanitest94',
    password: '729071',
    isAssigned: false,
  },
  {
    id: '95',
    username: 'Vanitest95',
    password: '242053',
    isAssigned: false,
  },
  {
    id: '96',
    username: 'Vanitest96',
    password: '662309',
    isAssigned: false,
  },
  {
    id: '97',
    username: 'Vanitest97',
    password: '733474',
    isAssigned: false,
  },
  {
    id: '98',
    username: 'Vanitest98',
    password: '330168',
    isAssigned: false,
  },
  {
    id: '99',
    username: 'Vanitest99',
    password: '466182',
    isAssigned: false,
  },
]

// 内存中的账号状态
let betaAccounts: BetaAccount[] = []

// 加载账号状态
async function loadAccountsState(): Promise<void> {
  try {
    const data = await fs.readFile(ACCOUNTS_STATE_FILE, 'utf-8')
    const savedAccounts = JSON.parse(data)
    
    // 恢复Date对象
    betaAccounts = savedAccounts.map((account: any) => ({
      ...account,
      assignedAt: account.assignedAt ? new Date(account.assignedAt) : undefined
    }))
  } catch (error) {
    // 如果文件不存在或读取失败，使用初始状态
    console.log('账号状态文件不存在，使用初始状态')
    betaAccounts = [...initialBetaAccounts]
    await saveAccountsState()
  }
}

// 保存账号状态
async function saveAccountsState(): Promise<void> {
  try {
    await fs.writeFile(ACCOUNTS_STATE_FILE, JSON.stringify(betaAccounts, null, 2))
  } catch (error) {
    console.error('保存账号状态失败:', error)
  }
}

// 确保账号状态已加载
let isLoaded = false
async function ensureLoaded(): Promise<void> {
  if (!isLoaded) {
    await loadAccountsState()
    isLoaded = true
  }
}

// 获取下一个可用的内测账号
export async function getNextAvailableAccount(): Promise<BetaAccount | null> {
  await ensureLoaded()
  return betaAccounts.find(account => !account.isAssigned) || null
}

// 分配账号给用户
export async function assignAccount(accountId: string, userEmail: string): Promise<boolean> {
  await ensureLoaded()
  const account = betaAccounts.find(acc => acc.id === accountId)
  if (account && !account.isAssigned) {
    account.isAssigned = true
    account.assignedTo = userEmail
    account.assignedAt = new Date()
    await saveAccountsState()
    return true
  }
  return false
}

// 检查邮箱是否已经分配过账号
export async function isEmailAlreadyAssigned(email: string): Promise<BetaAccount | null> {
  await ensureLoaded()
  return betaAccounts.find(account => account.assignedTo === email) || null
}

// 获取可用账号数量
export async function getAvailableAccountsCount(): Promise<number> {
  await ensureLoaded()
  return betaAccounts.filter(account => !account.isAssigned).length
}

// 获取所有账号状态（管理员功能）
export async function getAllAccounts(): Promise<BetaAccount[]> {
  await ensureLoaded()
  return [...betaAccounts]
}

// 重置账号状态（管理员功能）
export async function resetAccountsState(): Promise<void> {
  betaAccounts = [...initialBetaAccounts]
  await saveAccountsState()
} 